// Packages
import { ApiStatusEnums } from '@/services/react-query';
...
export const reactQueryUtilService = (function () {
 ...
 const isResponseOk = (status: number) => status === ApiStatusEnums.SUCCESS;

 const isErrorResponse = (status: number) => status !== ApiStatusEnums.INTERNAL_SERVER_ERROR;
